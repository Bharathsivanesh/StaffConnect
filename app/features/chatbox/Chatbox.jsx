...
export default function ChatScreen() {
  ...
  const chatId = getChatId(studentId, staffUid);
  ...
  const handleSend = async () => {
    ...
    // 2️⃣ Update chat document (for staff chat list)
    ...
    <SafeAreaView className="flex-1 bg-green-500">
      <View className="flex-1 bg-gray-100">
        ...
        <View className="flex-row items-center bg-green-500 p-4">
          ...
          <View>
            <Text className="text-white font-semibold">{staffname}</Text>
            <Text className="text-white text-xs">Online</Text>
          </View>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View
                className={`mb-3 ${isStudent ? "items-end" : "items-start"}`}
              >
                <View
                  className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                    isStudent
